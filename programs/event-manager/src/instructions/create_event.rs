...
pub struct CreateEvent<'info> {
    #[account(init,
        seeds = [Event::SEED_EVENT.as_ref(), // event
    ...
    #[account(init,
         seeds = [Event::SEED_EVENT_MINT.as_ref(), // event_mint
         event.key().as_ref()], // event public key
         bump,
         payer= authority,
         mint::decimals = 0, // not decimals
    ...
    #[account(init,
        seeds = [Event::SEED_TREASURY_VAULT.as_ref(), // treasury_vault
        event.key().as_ref()], // event public key
        bump,
        payer= authority,
    ...
    #[account(init,
        seeds = [Event::SEED_GAIN_VAULT.as_ref(), // gain_vault
        event.key().as_ref()], // event public key
        bump,
        payer= authority,
